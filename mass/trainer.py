...
#from fairseq.meters import meters.AverageMeter, meters.StopwatchMeter, meters.TimeMeter
...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch=None, oom_batch=None):
        ...
        self._reward_criterion = criterion
        if args.fp16:
            self._criterion = self._criterion.half()
            self._reward_criterion = self._reward_criterion.half()
            self._model = self._model.half()
        if self.cuda:
            self._criterion = self._criterion.cuda()
            self._reward_criterion = self._reward_criterion.cuda()
        ...
        self._wrapped_reward_criterion = None
    ...
    def init_meters(self, args):
        ...
        self.meters['train_loss'] = meters.AverageMeter()
        self.meters['train_reward_loss'] = meters.AverageMeter()
        self.meters['train_nll_loss'] = meters.AverageMeter()
        self.meters['valid_loss'] = meters.AverageMeter()
        self.meters['valid_reward_loss'] = meters.AverageMeter()
        self.meters['valid_nll_loss'] = meters.AverageMeter()
        self.meters['wps'] = meters.TimeMeter()       # words per second
        self.meters['ups'] = meters.TimeMeter()       # updates per second
        self.meters['wpb'] = meters.AverageMeter()    # words per batch
        self.meters['bsz'] = meters.AverageMeter()    # sentences per batch
        self.meters['gnorm'] = meters.AverageMeter()  # gradient norm
        self.meters['clip'] = meters.AverageMeter()   # % of updates clipped
        self.meters['oom'] = meters.AverageMeter()    # out of memory
        if args.fp16:
            self.meters['loss_scale'] = meters.AverageMeter()  # dynamic loss scale
        self.meters['wall'] = meters.TimeMeter()      # wall time in seconds
        self.meters['train_wall'] = meters.StopwatchMeter()  # train wall time in seconds
    ...
    @property
    def reward_criterion(self):
        if self._wrapped_reward_criterion is None:
            if (
                    utils.has_parameters(self._reward_criterion)
                    and self.args.distributed_world_size > 1
                    and not self.args.use_bmuf
            ):
                self._wrapped_reward_criterion = models.DistributedFairseqModel(
                    self.args, self._reward_criterion
                )
            else:
                self._wrapped_reward_criterion = self._reward_criterion
        return self._wrapped_reward_criterion
    ...
    def load_checkpoint(
        ...
        try:
            from fairseq import fb_pathmgr
        ...
        if extra_state is not None:
            ...
            if 'train_meters' in extra_state and not reset_meters:
                ...
                # reset meters.TimeMeters, since their start times don't make sense anymore
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
            ...
            try:
                ...
                if not ignore_grad:
                    ...
                    if self.fast_stat_sync:
                        ...
                        self._all_reduce_list[3] += logging_output.get('reward_loss', 0.0)
                        self._all_reduce_list[4] += logging_output.get('nll_loss', 0.0)
                        self._all_reduce_list[5] += logging_output.get('ntokens', 0.0)
        ...
        try:
            ...
            if 'reward_loss' in logging_output:
                self.meters['train_reward_loss'].update(logging_output.get('reward_loss', 0), sample_size)
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        self.meters['valid_reward_loss'].update(logging_output.get('reward_loss', 0), sample_size)
